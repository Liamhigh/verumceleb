...
const log=pino({level:'info'}); const app=express(); app.use(express.json({limit:'4mb'})); app.use(helmet({contentSecurityPolicy:false})); app.use(cors({ origin: true }));
...
(function verifyImmutablePack(){
  ...
  // Block extra files in rules dir (except manifest.json)
...
app.post('/v1/anchor', (req,res)=>{ const { hash }=req.body||{}; if(!hash) return res.status(400).json({ok:false,error:'invalid_hash'}); const r={ ok:true, hash, issuedAt:new Date().toISOString() }; putReceipt(hash,r); res.json(r); });
